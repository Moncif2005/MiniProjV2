...
class JobCard extends StatelessWidget {
  ...
  const JobCard({
    super.key,
    this.title, this.company, this.type, this.salary, this.location,
    this.onBookmark, this.onTap, this.offer,
    this.isRecruiter = false, this.isOwner = false,
    this.onApply, this.onWithdraw, this.onManage, this.onAvatarTap,
  });
        ...
        child: Column(
          children: [
            ...
            Container(
              height: 3,
              decoration: BoxDecoration(
                gradient: LinearGradient(colors: [typeColor, typeColor.withOpacity(0.4)]),
                ...
                children: [
                    ...
                    children: [
                      ...
                      Expanded(
                        child: Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                            Text(displayTitle, style: TextStyle(color: c.textPrimary, fontSize: 16, fontFamily: 'Inter', fontWeight: FontWeight.w700, height: 1.2)),
                            const SizedBox(height: 3),
                            Text(displayCompany, style: TextStyle(color: c.textSecondary, fontSize: 13, fontFamily: 'Inter', fontWeight: FontWeight.w500)),
                          ],
                        ),
                      ),
                  ...
                  Wrap(
                    ...
                    children: [
                      ...
                      _DetailPill(icon: null, text: displaySalary, color: AppColors.green), 
                      _DetailPill(icon: Icons.location_on_outlined, text: displayLocation, color: AppColors.primary),
...
_ExpandableText(
                    ...
                    children: [
                      if (_isDynamic) ...[
                        if (isRecruiter && isOwner && onManage != null)
                          Expanded(child: _ActionButton(label: 'Manage', icon: Icons.manage_accounts_rounded, color: AppColors.purple, onTap: onManage, filled: false)),
                        if (!isRecruiter)
                          Expanded(
                            child: FutureBuilder<bool>(
                              ...
                              builder: (ctx, snap) {
                                if (snap.connectionState == ConnectionState.waiting) {
                                  return SizedBox(height: 44, child: Center(child: SizedBox(width: 18, height: 18, child: CircularProgressIndicator(strokeWidth: 2, color: c.primary))));
                                }
  ...
  Widget _buildStatusBadges(ThemeColors c, bool isActive, String? status) {
    ...
    return Row(mainAxisSize: MainAxisSize.min, children: [
      ...
      if (status == 'approved' || (status == null && isActive)) _StatusBadge(label: 'Active', color: AppColors.green, icon: Icons.check_circle_rounded),
    ]);
  }
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          boxShadow: filled ? [BoxShadow(color: color.withOpacity(0.3), blurRadius: 10, offset: const Offset(0, 4))] : [],
        ),
        child: Row(
...
class _ExpandableText extends StatefulWidget {
  ...
  final Color color;
  final int maxLines;
  const _ExpandableText({required this.text, required this.color, this.maxLines = 3});
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.text.length < 120) {
      return Container(
        ...
        child: Text(widget.text, style: TextStyle(color: widget.color, fontSize: 12, fontFamily: 'Inter', height: 1.5)),
      );
    }
      ...
      child: Column(
        ...
        children: [
          Text(
            ...
            style: TextStyle(color: widget.color, fontSize: 12, fontFamily: 'Inter', height: 1.5),
          ),
          TextButton(
            onPressed: () => setState(() => _expanded = !_expanded),
            style: TextButton.styleFrom(padding: EdgeInsets.zero, minimumSize: Size.zero, tapTargetSize: MaterialTapTargetSize.shrinkWrap),
            child: Text(_expanded ? 'Show less' : 'Read more', style: TextStyle(color: AppColors.primary, fontSize: 12, fontWeight: FontWeight.w600, fontFamily: 'Inter')),
          ),
        ],
      ),
    );
