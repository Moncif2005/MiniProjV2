...
class _AuthenticatedRouterState extends State<_AuthenticatedRouter> {
  ...
  Future<void> _loadUserData() async {
    try {
      ...
      if (doc.exists) {
        final data = doc.data()!;
        userProvider.updateFromFirestore(data); // دالة التحديث الشامل
        final r = data['role']?.toString().toLowerCase();
        if (r == 'enseignant') {
          _role = UserRole.enseignant;
        } else if (r == 'recruteur') _role = UserRole.recruteur;
        else _role = UserRole.etudiant;
      } else {
  ...
  Widget build(BuildContext context) {
    ...
    // 🎯 التوجيه النهائي: كل دور يذهب لصفحته الخاصة
    switch (_role) {
